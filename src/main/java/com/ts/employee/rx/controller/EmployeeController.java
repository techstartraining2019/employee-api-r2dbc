...
public class EmployeeController {
  ...
  public Mono<ResponseEntity<Employee>> getEmployeeById(@PathVariable(name = "id") Integer id) {
  ...
  public Mono<ResponseEntity<Void>> deleteEmployee(@PathVariable(name = "id") Integer id) {
  ...
  @GetMapping("/api/employee/runtimeException")
  public Flux<Employee> runtimeException() {
    return employeeService.findAll()
        .concatWith(Mono.error(new RuntimeException("RuntimeException Occurred.")));
  }

  // if single controller then use @ExceptionHandler to handle runtime exception
